...
//----Bus_hand / RAM / Transform module    
    output wire         WriteCacheFlag_o   ,//modified by yb 2021-01-27
...
reg  WriteCacheFlag;  //modified by yb 2021-01-26 add for write bus
    ...
    case(current_state_reg)
      ...
      WriteBus:
            ...
            //     next_state_reg = WriteBus;
             if (WriteCacheFlag&&(RAM_WB_RB_cond))
                next_state_reg = Idle;
             else if(RAM_WB_RB_cond)                         
                next_state_reg = ReadBus;      
             
             else                                      
                next_state_reg = WriteBus;  //modified by yb 2021-01-26
      ReadBus:
             if(RAM_RB_WC_cond)                         
      ...
      WriteCache:                                       
               ...
               if (current_state2_reg == Idle)  //modified by yb 2021-01-26
                  next_state_reg = WriteBus;
               else
                  next_state_reg = Idle;
      default:                                          
...
assign      readbus_en         = (FSM_current_state==ReadBus)  ?1:0;   
...
//=======modified by yb 2021-01-26 add for WriteCache======

always @(posedge clk or negedge rst_n) begin
  if(!rst_n)    
      WriteCacheFlag <= 0;
  else if ((current_state2_reg == WriteCache)&&(current_state_reg == WriteBus))
      WriteCacheFlag <= 1;
  else if ((current_state2_reg == WriteBus) && (current_state_reg != WriteBus))         
      WriteCacheFlag <= 0;

end
assign WriteCacheFlag_o=WriteCacheFlag;
endmodule