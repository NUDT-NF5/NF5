...
`define All_CTRL_WIDTH	`A_SEL_WIDTH + `B_SEL_WIDTH + `ALU_OP_WIDTH + `ST_TYPE_WIDTH + `LD_TYPE_WIDTH + `WB_SEL_WIDTH + `BOOL_WIDTH + `CSR_CMD_WIDTH + `BOOL_WIDTH + 1
`define localAllCTRLWIDTH32  `PC_SEL_WIDTH + `A_SEL_WIDTH + `B_SEL_WIDTH + `IMM_SEL_WIDTH+ `ALU_OP_WIDTH + `BR_TYPE_WIDTH+ `BOOL_WIDTH + `ST_TYPE_WIDTH+ `LD_TYPE_WIDTH+ `WB_SEL_WIDTH + `BOOL_WIDTH+ `CSR_CMD_WIDTH+ `BOOL_WIDTH+ `DECODE_STALL_WIDTH + `DECODE_FLUSH_WIDTH + `XLEN_WIDTH_SEL + 1
...
`define PIPE_IDEX_LEN  `All_CTRL_WIDTH + `RF_ADDR_WIDTH + `RF_ADDR_WIDTH + `RF_ADDR_WIDTH + `DATA_WIDTH + `IMM_SEL_WIDTH + `CSR_ADDR_WIDTH + 1 + `ADDR_WIDTH + 1
