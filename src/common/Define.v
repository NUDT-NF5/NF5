...
`define FPU_FADD        `ALU_OP_WIDTH'd30
`define FPU_FSUB        `ALU_OP_WIDTH'd31
`define FPU_FMUL        `ALU_OP_WIDTH'd32
`define FPU_FDIV        `ALU_OP_WIDTH'd33
`define FPU_FMIN_MAX    `ALU_OP_WIDTH'd34
`define FPU_FSQRT       `ALU_OP_WIDTH'd35
`define FPU_FMADD       `ALU_OP_WIDTH'd36
`define FPU_FMSUB       `ALU_OP_WIDTH'd37
`define FPU_FNMSUB      `ALU_OP_WIDTH'd38
`define FPU_FNMADD      `ALU_OP_WIDTH'd39
`define FPU_FCVT_F2I    `ALU_OP_WIDTH'd40
`define FPU_FCVT_F2IU   `ALU_OP_WIDTH'd41
`define FPU_FCVT_I2F    `ALU_OP_WIDTH'd42
`define FPU_FCVT_IU2F   `ALU_OP_WIDTH'd43
`define FPU_FCVT_F2F    `ALU_OP_WIDTH'd44
`define FPU_FSGNJ       `ALU_OP_WIDTH'd45
`define FPU_FMV_F2X     `ALU_OP_WIDTH'd46
`define FPU_FMV_X2F     `ALU_OP_WIDTH'd47
`define FPU_FCMP        `ALU_OP_WIDTH'd48
`define FPU_FCLASS      `ALU_OP_WIDTH'd49
...
`define PIPE_IDEX_LEN 	`All_CTRL_WIDTH + `RF_ADDR_WIDTH + `RF_ADDR_WIDTH + `RF_ADDR_WIDTH + `DATA_WIDTH + `IMM_SEL_WIDTH + `CSR_ADDR_WIDTH +`DATA_WIDTH + `DATA_WIDTH + `DATA_WIDTH + 1 + `ADDR_WIDTH + `FUNCT3_WIDTH + 2
`define PIPE_EXMem_LEN 	32 + 32 + `RF_ADDR_WIDTH + `DATA_WIDTH + `ST_TYPE_WIDTH + `LD_TYPE_WIDTH + `BOOL_WIDTH + `WB_SEL_WIDTH
`define PIPE_MemWb_LEN 	`RF_ADDR_WIDTH + 1 + `DATA_WIDTH + `DATA_WIDTH + 1
