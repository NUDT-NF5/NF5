...
// `define PIPE_IDEX_ISSUE1_START `All_CTRL_WIDTH + `RF_ADDR_WIDTH + `RF_ADDR_WIDTH + `RF_ADDR_WIDTH + `DATA_WIDTH + `IMM_SEL_WIDTH + `CSR_ADDR_WIDTH +`DATA_WIDTH + `DATA_WIDTH + 1 + `ADDR_WIDTH
...
// `define PIPE_EXMem_ISSUE1_START `DATA_WIDTH + `RF_ADDR_WIDTH + `DATA_WIDTH + `ST_TYPE_WIDTH + `LD_TYPE_WIDTH + `BOOL_WIDTH + `WB_SEL_WIDTH + `ADDR_WIDTH
// `define PIPE_MemWb_LEN 	(`RF_ADDR_WIDTH + `BOOL_WIDTH + `DATA_WIDTH + `DATA_WIDTH + `BOOL_WIDTH) * 2
// `define PIPE_MemWb_ISSUE1_START `RF_ADDR_WIDTH + `BOOL_WIDTH + `DATA_WIDTH + `DATA_WIDTH + `BOOL_WIDTH
...
`define PIPE_EXMem_LEN 	`DATA_WIDTH + `RF_ADDR_WIDTH + `DATA_WIDTH + `ST_TYPE_WIDTH + `LD_TYPE_WIDTH + `BOOL_WIDTH + `WB_SEL_WIDTH + `ADDR_WIDTH
`define PIPE_MemWb_LEN 	`RF_ADDR_WIDTH + `BOOL_WIDTH + `DATA_WIDTH + `DATA_WIDTH + `BOOL_WIDTH
